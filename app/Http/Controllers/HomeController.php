...
class HomeController extends Controller
{
    ...
    {
        $title = 'Melbourne migration agency | work visa australia | Visawizer';
        $description = 'Trusted Melbourne migration agency offering expert visa support for student, work, partner, PR, and business visas across Australia.';
        $keywords = ' migration agency Melbourne, certified Melbourne migration agent, visa services Melbourne, Australia migration consultant, work visa Australia, business visa consultant, migration advice Melbourne, visa application assistance';
        $banners = \DB::table('banners')->where('status', 1)->orderBy('orderNo', 'asc')->get();
        return view('index', compact('banners', 'title', 'description', 'keywords'));
    }

    public function studyInAustralia()
    {
        $page = StudyTopicData::get('study-visas') ?? [];
        extract(StudyTopicData::resolveSeo($page));

        return view('study', compact('title', 'description', 'keywords'));
    }
    ...
    public function visaGuide()
    {
        ...
        return view('visa-guide-main', compact('title', 'description', 'keywords'));
    }

    public function bookAppointment()
    {
        $appointmentPage = BookAppointmentData::page();
        $countryDialCodes = CountryDialCodes::forSelect();
        $title = $appointmentPage['meta']['title'];
        $description = $appointmentPage['meta']['description'];
        $keywords = $appointmentPage['meta']['keywords'];

        return view('bookAppointment', compact('appointmentPage', 'countryDialCodes', 'title', 'description', 'keywords'));
    }
    ...
    public function workHoliday462Landing()
    {
        $title = 'Australia Work and Holiday Visa Subclass 462 | Apply with Expert Guidance | Visawizer';
        $description = 'Apply for Australia Work and Holiday Visa Subclass 462 with Visawizer. Check eligibility, prepare documents, and get expert guidance for young travellers from participating countries.';
        $keywords = 'work and holiday visa 462, working holiday visa subclass 462, 462 visa Australia, work and holiday visa apply, Visawizer';

        return view('workHoliday462Landing', compact('title', 'description', 'keywords'));
    }
    ...
    private function renderStudyTopicPage(array $page)
    {
        ...
        return view('studyTopic', compact('page', 'countries', 'title', 'description', 'keywords'));
    }
    ...
    private function renderVisaTopicPage(array $visaPage)
    {
        ...
        $title = $meta['title'] ?? (($visaPage['label'] ?? 'Visa') . ' | ' . $sectionTitle . ' | Visawizer');
        $rawDescription = $visaPage['hero']['subheading'] ?? $visaPage['hero']['content'] ?? '';
        $description = $meta['description'] ?? Str::limit(strip_tags($rawDescription), 300, '');
        $keywords = $meta['keywords'] ?? (match ($sectionTitle) {
    ...
    public function whitePapers()
    {
        ...
        return view('whitePapers', compact('whitePaper'));
    }
    ...
    public function caseStudies()
    {
        ...
        return view('caseStudies', compact('caseStudy'));
    }

    public function singleCaseStudy($slug)
    {
        $caseStudy = CaseStudy::where('slug', $slug)->firstOrFail();
        return view('singleCaseStudy', compact('caseStudy'));
    }
    ...
    public function blogs(Request $request)
    {
        ...
        $perPage = 12;
        $categoryId = 2;
        $response = Http::get("https://visawizer.com.au/visawizer-blog/wp-json/wp/v2/posts?_embed&per_page={$perPage}&page={$page}&categories={$categoryId}");
    ...
    public function blogsByCategory(Request $request, $slug)
    {
        ...
        $categories = Http::get('https://visawizer.com.au/visawizer-blog/wp-json/wp/v2/categories')->json();
        ...
        $page = $request->get('page', 1);
        $perPage = 12;
        $response = Http::get("https://visawizer.com.au/visawizer-blog/wp-json/wp/v2/posts?categories={$catId}&_embed&per_page={$perPage}&page={$page}");
    ...
    public function blogSingle($slug)
    {
        ...
        $description = 'Visawizer';
        $keywords = 'Visawizer';
        $res = Http::get("https://visawizer.com.au/visawizer-blog/wp-json/wp/v2/posts?slug=$slug&_embed");
